...
def Overlap(*hypervectors: ArrayLike):
    ...
    means the best results will be achieved from passing the bundled
    ...
    if len(hvs) == 2:
        ...
        if is_torch:
            ...
            if a_t.ndim == 2:
                return mask.sum(dim=1).float() / b_t.sum(dim=1).float().clamp(min=1)
            return mask.sum().item() / max(b_t.sum().item(), 1)
        else:
            a_n, b_n = np.asarray(a), np.asarray(b)
            mask = np.logical_and(a_n == b_n, a_n == 1)
            if a_n.ndim == 2:
                return mask.sum(axis=1) / np.maximum(b_n.sum(axis=1), 1)
            return int(mask.sum()) / max(int(b_n.sum()), 1)
    else:
        arr = hvs[0]
        ...
        if is_torch:
            ...
            return mask.sum(dim=1).float() / max(ref_t.sum().item(), 1)
        else:
            rest_n, ref_n = np.asarray(rest), np.asarray(ref)
            mask = np.logical_and(rest_n == ref_n, rest_n == 1)
            return mask.sum(axis=1) / max(int(ref_n.sum()), 1)
