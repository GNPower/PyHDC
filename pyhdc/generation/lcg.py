    ...
    def set_parameters(
        ...
        if multiplier is not None:
            if (
                ...
                or multiplier < 0
                or multiplier >= self._modulus
            ):
                raise ValueError(
                    f"Multiplier must be between 0 and {self._modulus - 1}"
                )
            self._multiplier = multiplier
...
class CommonLCGGenerators:
    ...
    def numerical_recipes(seed: Optional[int] = None) -> LCGGenerator:
        ...
        return LCGGenerator(
            modulus=2**32, multiplier=1664525, increment=1013904223, seed=seed
        )
    ...
    def park_miller(seed: Optional[int] = None) -> MultiplicativeLCGGenerator:
        ...
        return MultiplicativeLCGGenerator(
            modulus=2**31 - 1, multiplier=16807, seed=seed
        )
    ...
    def minstd(seed: Optional[int] = None) -> MultiplicativeLCGGenerator:
        ...
        return MultiplicativeLCGGenerator(
            modulus=2**31 - 1, multiplier=48271, seed=seed
        )

    @staticmethod
    def borland(seed: Optional[int] = None) -> LCGGenerator:
        """Borland C/C++ LCG."""
        return LCGGenerator(modulus=2**32, multiplier=22695477, increment=1, seed=seed)

    @staticmethod
    def msvc(seed: Optional[int] = None) -> LCGGenerator:
        """Microsoft Visual C++ LCG."""
        return LCGGenerator(
            modulus=2**32, multiplier=214013, increment=2531011, seed=seed
        )
